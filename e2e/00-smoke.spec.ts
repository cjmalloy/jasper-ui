...
test.describe.serial('Smoke Tests', () => {
  ...
  test('loads the page', async () => {
    ...
    await expect(page.getByText('Powered by Jasper')).toBeVisible({ timeout: 1000 * 60 * 20 }); // 20 minutes
  });
  ...
  test('loads the ADMIN user', async () => {
    const whoamiPromise = page.waitForResponse(resp => resp.url().includes('/api/v1/user/whoami') && resp.request().method() === 'GET');
    await page.goto('/?debug=ADMIN');
