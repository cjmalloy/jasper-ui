    ...
    script: `
      ...
      for (const c of sources) {
        ...
        for (const media of mediaTypes) {
          ...
          if (!media.config || !config[media.config]) {
            attachLog(response, config.provider + ' (' + config.model + ') does not support ' + media.plugin + ' input.');
            continue;
          }
          ...
          try {
            ...
            attachLog(response, 'Could not load ' + media.plugin + ' input from ' + url + '.');
          }
        }
      ...
      try {
        ...
        for (const media of mediaMessages) {
          ...
          attachLog(response, config.provider + ' (' + config.model + ') rejected the media input.');
        }
        generated = await provider.generate(messages, config);
      }
      ...
      response.plugins ||= {};
      Object.assign(response.plugins, completionRef.plugins || {});
      response.plugins['plugin/llm'] = {
      ...
      const uploadPart = async (partNumber, file) => {
        if (!uploadedFiles.has(partNumber)) {
          uploadedFiles.set(partNumber, await uploadFile(file));
        }
        return uploadedFiles.get(partNumber);
      };
      ...
      for (let i = 0; i < bundle.ref.length; i++) {
        ...
        for (const media of mediaTypes) {
          ...
          const cache = await uploadPart(Number(mediaPart[1]), mediaFile);
        ...
        const file = part ? files?.[Number(part[1]) - 1] : undefined;
        if (file) {
          const cache = await uploadPart(Number(part[1]), file);
