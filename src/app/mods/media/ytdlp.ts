...
if 'entries' in info:
    ...
    for index, entry in enumerate(info['entries']):
        ...
        child_ref = {
            ...
            'tags': ['plugin/embed'] + child_tags,
            'plugins': {'plugin/embed': {'url': entry_url}},
        }
        apply_metadata(child_ref, entry)
        output_refs.append(child_ref)
        source_urls.append(entry_url)
    ref['sources'] = source_urls
    ref['tags'] = [t for t in parent_tags if not (t + '/').startswith('plugin/embed/')]
    if 'plugin/playlist' not in ref['tags']:
        ref['tags'].append('plugin/playlist')
    ref.setdefault('plugins', {}).pop('plugin/embed', None)
else:
    apply_metadata(ref, info)
