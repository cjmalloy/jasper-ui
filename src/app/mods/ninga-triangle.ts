  ...
  config: {
    ...
    <script>
      ...
      Handlebars.registerHelper('ninjaTriangle', (comment, actions, el, d3) => () => {
        ...
        svg.selectAll('g')
          .data(ds, d => d.i + ':' + d.j + ':' + d.extraRow + ':' + d.red)
          .join(enter => {
          ...
          .on('click', (event, d) => {
            ...
            comment = tagRed(comment, d.i + 1, d.j);
            svg.selectAll('circle[row="' + d.i + '"]').attr('fill', '#CCC');
            svg.selectAll('text[row="' + d.i + '"]').text('');
            svg.selectAll('circle[row="' + d.i + '"][col="' + d.j + '"]').attr('fill', '#A33');
            for (let i = 0; i < d.n; i++) {
              for (let j = 0; j <= i; j++) {
                svg.selectAll('text[row="' + i + '"][col="' + j + '"]').text(bestNinjaPath(comment, i, j));
              }
            }
          })
          .on('contextmenu', (event, d) => {
            event.preventDefault();
            comment = removeRow(comment, d.n);
            svg.selectAll('circle[row="' + (d.n - 1) + '"]').attr('fill', 'none');
            svg.selectAll('text[row="' + (d.n - 1) + '"]').style('fill', 'none');
            svg.selectAll('circle[row="' + d.n + '"]').attr('stroke', 'none');
          });
      });
    </script>
    `,
