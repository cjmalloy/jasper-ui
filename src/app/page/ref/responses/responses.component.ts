...
export class RefResponsesComponent implements OnInit {
  ...
  constructor(
    ...
    this.page$ = combineLatest(
      ...
      map(([url, sort, filter, search, pageNumber, pageSize]) =>
        getArgs('', sort, {...filterListToObj(filter), responses: url}, search, pageNumber, pageSize ?? this.defaultPageSize)),
  ...
  get url$() {
    return this.route.params.pipe(
      ...
      tap(url => this.refs.get(url).subscribe(ref => this.theme.setTitle('Responses: ' + (ref.title || ref.url)))),
    );
  }
